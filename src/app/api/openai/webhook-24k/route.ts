// OpenAI Realtime SIP webhook handler - 24kHz VERSION
// NOTE: Poskus z višjo sample rate (24kHz) če je problem v downsampling
...
export async function POST(req: Request): Promise<Response> {
  try {
    ...
    if (event.type === 'realtime.call.incoming') {
      console.log('[openai-webhook-24k] incoming call event', JSON.stringify(event, null, 2));
      ...
      // 24kHz VERSION: Višja sample rate za boljšo kvaliteto
      const acceptPayload = {
        type: 'realtime',
        model: MODEL,
        ...
        audio: { 
          ...
          output: { voice: VOICE, format: 'pcm16', sample_rate: 24000 }
        },
        turn_detection: { type: 'semantic_vad' },
      } as const;

      console.log('[openai-webhook-24k] accept payload for', callId, JSON.stringify(acceptPayload, null, 2));
      ...
      if (!resAccept.ok) {
        ...
        console.error('[openai-webhook-24k] accept failed', resAccept.status, txt);
        return new Response('Accept failed', { status: 500 });
      }
      console.log('[openai-webhook-24k] accept ok for', callId);
      ...
      ;(async () => {
        try {
          ...
          ws.on('open', () => {
            console.log('[openai-webhook-24k] ws open for', callId);
            try {
              ws.send(
                JSON.stringify({
              ...
              console.log('[openai-webhook-24k] response.create sent');
            } catch (e) {
              console.warn('[openai-webhook-24k] response.create failed', e);
            }
          });
          ws.on('message', (m) => {
            try {
              ...
              if (ev?.type === 'error') {
                console.error('[openai-webhook-24k] WS ERROR:', JSON.stringify(ev, null, 2));
              } else if (ev?.type === 'session.created' || ev?.type === 'session.updated') {
                console.log('[openai-webhook-24k] SESSION EVENT:', ev?.type);
                if (ev?.session?.input_audio_format) {
                  console.log('[openai-webhook-24k] Negotiated input format:', ev.session.input_audio_format);
                }
                if (ev?.session?.output_audio_format) {
                  console.log('[openai-webhook-24k] Negotiated output format:', ev.session.output_audio_format);
                }
              } else if (ev?.type === 'response.audio_transcript.done') {
                console.log('[openai-webhook-24k] TTS Completed:', ev?.transcript || 'no transcript');
              } else {
                console.log('[openai-webhook-24k] ws event', ev?.type || 'unknown');
              }
            } catch {}
          });
          ws.on('error', (e) => console.warn('[openai-webhook-24k] ws error', e));
          // Pustimo odprto; strežnik lahko sam zaključi sejo po koncu klica
        } catch (err) {
          console.warn('[openai-webhook-24k] ws init failed', err);
        }
      })();
    ...
    console.error('[openai-webhook-24k] error', e);
